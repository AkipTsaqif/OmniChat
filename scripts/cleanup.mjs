...
const r = await sql`DELETE FROM users WHERE email LIKE 'probe%@example.com' OR email LIKE 'shot%@example.com' OR email LIKE 'models%@example.com' OR email LIKE 'dbg%@example.com' OR email LIKE 'pending%@example.com' OR email LIKE 'btn%@example.com' OR email LIKE 'dev%@example.com' RETURNING email`;
